...
func (p *Proxy) reportTunnelStatus(status int) {
	...
	if err != nil {
		// Log only on explicit OFFLINE reports (shutdown) -- online retries are silent.
...
func (p *Proxy) bindDevice() {
	...
	for attempt := 1; ; attempt++ {
		...
		resp, err := http.Post(bindURL, "application/json", bytes.NewBufferString(body))
		if err == nil {
...
func (p *Proxy) heartbeatLoop() {
	...
	for {
		...
		resp, err := http.Post(hbURL, "application/json", bytes.NewBufferString(body))
		if err != nil {
