...
# Auto-detect ALL LAN IPs -- handles single-NIC, dual-NIC, and multi-NIC devices
mapfile -t ALL_LAN_IPS < <(ip -4 addr show scope global 2>/dev/null | \
    grep -oP '(?<=inet\s)\d+(\.\d+){3}' | \
    grep -Ev '^(127\.|172\.(1[6-9]|2[0-9]|3[01])\.)' || true)

if [[ ${#ALL_LAN_IPS[@]} -eq 0 ]]; then
    # Fallback: hostname -I
    DETECTED_LAN_IP=$(hostname -I 2>/dev/null | awk '{print $1}' || echo "0.0.0.0")
    ALL_LAN_IPS=("${DETECTED_LAN_IP}")
    ok "Detected LAN IP: ${DETECTED_LAN_IP} (fallback)"
elif [[ ${#ALL_LAN_IPS[@]} -eq 1 ]]; then
    DETECTED_LAN_IP="${ALL_LAN_IPS[0]}"
    ok "Detected LAN IP: ${DETECTED_LAN_IP}"
else
    DETECTED_LAN_IP="${ALL_LAN_IPS[0]}"
    ok "Detected ${#ALL_LAN_IPS[@]} network interfaces: ${ALL_LAN_IPS[*]}"
fi
...
read_yaml() {
    # read_yaml KEY FILE
    # Uses awk for reliable parsing -- handles URLs with colons (e.g.
    # https://domain:443) that broke the old sed 's/.*://' approach.
    # Also handles YAML values with or without surrounding double quotes.
    local key="$1" file="$2"
...
if ($OPT_YES || $OPT_REINSTALL) && $CONFIG_EXISTS && ! $OPT_RECONFIGURE; then
    ...
    # -- Required: PBX Domain --
    # RINGQ_DOMAIN env var = called from provisioning API (non-interactive)
    if [[ -n "${RINGQ_DOMAIN:-}" ]]; then
        PBX_DOMAIN="${RINGQ_DOMAIN}"
        echo -e "  ${GREEN}[api]${NC}  PBX Domain: ${YELLOW}${PBX_DOMAIN}${NC}"
    elif $OPT_YES && [[ -n "${E_PBX_DOMAIN}" ]]; then
    ...
    # -- Required: Auth Key --
    # RINGQ_AUTH_KEY env var = called from provisioning API (non-interactive)
    if [[ -n "${RINGQ_AUTH_KEY:-}" ]]; then
        AUTH_KEY="${RINGQ_AUTH_KEY}"
        echo -e "  ${GREEN}[api]${NC}  Auth Key: ${YELLOW}${AUTH_KEY:0:12}...${NC}"
    elif $OPT_YES && [[ -n "${E_AUTH_KEY}" ]]; then
        AUTH_KEY="${E_AUTH_KEY}"
    ...
    # Always derive API URL from current PBX_DOMAIN so --reconfigure with a
    # new domain gets the correct default. In advanced mode the stored URL is
    # offered only when it looks like a real URL (not a broken "443" value).
    AUTO_API_URL="https://${PBX_DOMAIN}:443"
    if $OPT_ADVANCED && [[ "${E_API_URL}" == http* ]]; then
        AUTO_API_URL="${E_API_URL}"
    fi
    # Tunnel host follows PBX_DOMAIN on --reconfigure (domain just changed).
    # For --yes / --reinstall, preserve existing value.
    # In advanced mode, the adv_prompt below still lets the user override.
    if $OPT_RECONFIGURE; then
        AUTO_TUNNEL_HOST="${PBX_DOMAIN}"
    else
        AUTO_TUNNEL_HOST="${E_TUNNEL_HOST:-${PBX_DOMAIN}}"
    fi
    AUTO_TUNNEL_PORT="${E_TUNNEL_PORT:-6010}"
    # RINGQ_LAN_IP env var = API-provided LAN IP, skip multi-interface selection
    if [[ -n "${RINGQ_LAN_IP:-}" ]]; then
        AUTO_LAN_IP="${RINGQ_LAN_IP}"
        DETECTED_LAN_IP="${RINGQ_LAN_IP}"
        echo -e "  ${GREEN}[api]${NC}  LAN IP: ${YELLOW}${AUTO_LAN_IP}${NC}"
    else
        AUTO_LAN_IP="${E_LAN_IP:-${DETECTED_LAN_IP}}"
    fi

    # Multiple interfaces: ask user to select unless --yes with existing config
    if [[ ${#ALL_LAN_IPS[@]} -gt 1 ]]; then
        _USE_EXISTING=false
        ($OPT_YES || $OPT_REINSTALL) && [[ -f "${CONFIG_FILE}" ]] && \
            ! $OPT_RECONFIGURE && _USE_EXISTING=true
        if ! $_USE_EXISTING; then
            echo ""
            echo -e "  ${YELLOW}Multiple network interfaces detected.${NC}"
            echo -e "  ${YELLOW}Select the IP that LAN phones will connect to:${NC}"
            echo ""
            _DEF_IDX=0
            for _i in "${!ALL_LAN_IPS[@]}"; do
                _IP="${ALL_LAN_IPS[$_i]}"
                _IFACE=$(ip -4 addr show 2>/dev/null | grep "inet ${_IP}" | \
                    awk '{print $NF}' | head -1 || echo "")
                _MARKER=""; [[ "${ALL_LAN_IPS[$_i]}" == "${AUTO_LAN_IP}" ]] && \
                    { _MARKER=" ${GREEN}← current${NC}"; _DEF_IDX=$((_i+1)); }
                echo -e "    ${BOLD}$((_i+1))${NC}) ${YELLOW}${ALL_LAN_IPS[$_i]}${NC}${_IFACE:+  ($_IFACE)}${_MARKER}"
            done
            [[ $_DEF_IDX -eq 0 ]] && _DEF_IDX=1
            echo ""
            while true; do
                read -rp "  Enter number [${_DEF_IDX}]: " _SEL
                _SEL="${_SEL:-$_DEF_IDX}"
                if [[ "$_SEL" =~ ^[0-9]+$ ]] && \
                   (( _SEL >= 1 && _SEL <= ${#ALL_LAN_IPS[@]} )); then
                    AUTO_LAN_IP="${ALL_LAN_IPS[$((_SEL-1))]}"
                    DETECTED_LAN_IP="${AUTO_LAN_IP}"
                    ok "Selected Device IP: ${AUTO_LAN_IP}"
                    break
                fi
                echo -e "  ${RED}Invalid. Enter 1 to ${#ALL_LAN_IPS[@]}.${NC}"
            done
        else
            ok "Using existing Device IP: ${AUTO_LAN_IP} (--yes mode)"
        fi
    fi
...
NEED_PKG=()
for pkg in git curl ca-certificates iptables-persistent netfilter-persistent file iproute2 nftables python3; do
...
if $NEED_BUILD; then
    ...
    for candidate in "${BUILD_DIR}/${BINARY_NAME}" "${BUILD_DIR}/bin/${BINARY_NAME}"; do
        if [[ -f "${candidate}" ]]; then
            # GitHub strips execute bits on push — restore them before checking.
            chmod +x "${candidate}" 2>/dev/null || true
            # Detect ELF binary via magic bytes (works without 'file' command)
            if head -c 4 "${candidate}" 2>/dev/null | grep -q $'\x7fELF' ||                file "${candidate}" 2>/dev/null | grep -qi 'ELF'; then
                PREBUILT_BINARY="${candidate}"
                break
            fi
        fi
    done
...
# API auth port moved from a dedicated 8443 to the standard 443 -- this one
# rule now covers both the tunnel bind/heartbeat API and general HTTPS
# (RingQ portal), since they're the same port. No separate 8443 rule needed.
ipt_add OUTPUT -p tcp --dport 443  -j ACCEPT \
    -m comment --comment "RingQ-https-and-api"
...
# -- nftables RTP relay rule --------------------------------------------------
# On Debian/Ubuntu systems that use nftables with a default-drop input policy
# (e.g. if 3CX or another PBX stack was previously installed), iptables rules
# are invisible to nftables and phone RTP to relay ports is silently dropped.
# We detect this and insert the relay port range into the nftables input chain.
if command -v nft &>/dev/null 2>&1; then
    if nft list ruleset 2>/dev/null | grep -q 'hook input.*policy drop'; then
        if ! nft list ruleset 2>/dev/null | grep -q 'dport 40000-41999'; then
            nft insert rule inet filter input                 udp dport 40000-41999 accept                 comment "RingQ-RTP-relay" 2>/dev/null &&                 ok "nftables: RTP relay ports 40000-41999 opened (policy-drop chain)" ||                 warn "nftables: could not insert RTP relay rule (non-fatal)"
        else
            skip "nftables: RTP relay ports already open"
        fi
        # Persist to config file so it survives nftables service restarts
        if [[ -f /etc/nftables.conf ]]; then
            if ! grep -q "40000-41999" /etc/nftables.conf; then
                sed -i '/type filter hook input/a\t		udp dport 40000-41999 accept comment "RingQ-RTP-relay"'                     /etc/nftables.conf
                ok "nftables: RTP relay rule persisted to /etc/nftables.conf"
            fi
...
#!/bin/bash
# =============================================================================
# RingQ NX Device Proxy -- Update Script
# Works with pre-built binary OR Go source -- no Go required for binary repos
# =============================================================================
set -euo pipefail
REPO_URL="https://github.com/Cal4Care-Developers/proxytunnel.git"
...
CURRENT=$(cat "${INSTALL_DIR}/version.txt" 2>/dev/null || echo "none")
if [[ -n "${REMOTE}" && "${REMOTE}" == "${CURRENT}" ]]; then
    log "Already on latest version: ${CURRENT}"; exit 0
fi
log "Updating: ${CURRENT} -> ${REMOTE:-latest}"

# Backup current binary before stopping service
if [[ -x "${INSTALL_DIR}/${BINARY_NAME}" ]]; then
    cp "${INSTALL_DIR}/${BINARY_NAME}" "${ROLLBACK}"
    log "Rollback copy saved"
fi
systemctl stop "${SERVICE_NAME}" 2>/dev/null && log "Service stopped" || true

# Clone repo
rm -rf "${BUILD_DIR}"
git clone --depth 1 "${REPO_URL}" "${BUILD_DIR}" 2>&1 | grep -E 'Cloning|done\.' || true
VER=$(git -C "${BUILD_DIR}" rev-parse --short HEAD 2>/dev/null || echo "unknown")

# Detect what is in the repo: pre-built binary or source files?
PREBUILT=""
for candidate in "${BUILD_DIR}/${BINARY_NAME}" "${BUILD_DIR}/bin/${BINARY_NAME}"; do
    if [[ -f "${candidate}" ]] &&            ( head -c 4 "${candidate}" 2>/dev/null | grep -q $'\x7fELF' ||              file "${candidate}" 2>/dev/null | grep -qi 'ELF' ); then
        PREBUILT="${candidate}"; break
    fi
done
GO_FILE_COUNT=$(find "${BUILD_DIR}" -maxdepth 3 -name "*.go" 2>/dev/null | wc -l)

if [[ -n "${PREBUILT}" ]]; then
    # Mode 1: Pre-built binary in repo -- copy directly, no Go needed
    log "Pre-built binary found -- copying directly (no Go required)"
    cp "${PREBUILT}" "${INSTALL_DIR}/${BINARY_NAME}.new"
    chmod +x "${INSTALL_DIR}/${BINARY_NAME}.new"

elif [[ "${GO_FILE_COUNT}" -gt 0 ]]; then
    # Mode 2: Source files in repo -- build from source
    warn "No pre-built binary -- attempting source build (${GO_FILE_COUNT} .go files)"
    export PATH="/usr/local/go/bin:${PATH}"; export GOPATH="/root/go"
    if ! command -v go &>/dev/null; then
        err "Go not installed and no pre-built binary found."
        err "Push compiled sipproxy binary to repo OR install Go: https://go.dev/dl/"
        [[ -f "${ROLLBACK}" ]] && mv "${ROLLBACK}" "${INSTALL_DIR}/${BINARY_NAME}"
        systemctl start "${SERVICE_NAME}" 2>/dev/null || true
        rm -rf "${BUILD_DIR}"; exit 1
    fi
    cd "${BUILD_DIR}"
    if [[ ! -f "go.mod" ]]; then
        FOUND=$(find . -maxdepth 3 -name "go.mod" | head -1)
        if [[ -n "${FOUND}" ]]; then cd "$(dirname ${FOUND})"
        else go mod init github.com/ochinchina/sipproxy && go mod tidy; fi
    fi
    GOOS=linux go build -ldflags="-s -w" -o "${INSTALL_DIR}/${BINARY_NAME}.new" .

else
    # Mode 3: Nothing usable in repo
    err "Repo has no pre-built binary and no Go source files."
    err "Push compiled sipproxy to: ${REPO_URL}"
    [[ -f "${ROLLBACK}" ]] && mv "${ROLLBACK}" "${INSTALL_DIR}/${BINARY_NAME}"
    systemctl start "${SERVICE_NAME}" 2>/dev/null || true
    rm -rf "${BUILD_DIR}"; exit 1
fi

# Verify the binary is a real ELF executable
if ! file "${INSTALL_DIR}/${BINARY_NAME}.new" 2>/dev/null | grep -qi 'ELF'; then
    err "Result is not a valid executable -- aborting"
...
rm -f "${ROLLBACK}"; rm -rf "${BUILD_DIR}"; cd /
log "Binary updated to v${VER}"

# Start service
systemctl start "${SERVICE_NAME}"; sleep 3
systemctl is-active --quiet "${SERVICE_NAME}" && \
    log "Service running -- version: ${VER}" || \
    { err "Service failed -- check: journalctl -u ${SERVICE_NAME} -n 30"; exit 1; }
UPDATESCRIPT
...
# -- ringqtunnel-status command -----------------------------------------------
section "Status Command"
INSTALL_PHASE="status-command"

STATUS_CMD="/usr/local/bin/ringqtunnel-status"
STATUS_URL="https://raw.githubusercontent.com/Cal4Care-Developers/proxytunnel/master/ringqtunnel-status"

if curl -fsSL --retry 3 --retry-delay 5 "${STATUS_URL}" -o "${STATUS_CMD}.new" 2>/dev/null; then
    chmod +x "${STATUS_CMD}.new"
    if [[ -f "${STATUS_CMD}" ]] && diff -q "${STATUS_CMD}" "${STATUS_CMD}.new" &>/dev/null; then
        rm -f "${STATUS_CMD}.new"
        skip "ringqtunnel-status (already up to date)"
    else
        mv "${STATUS_CMD}.new" "${STATUS_CMD}"
        ok "Status command installed: ${STATUS_CMD}"
    fi
else
    rm -f "${STATUS_CMD}.new" 2>/dev/null || true
    warn "Could not download ringqtunnel-status -- install manually from GitHub"
fi
...
echo -e "  Tunnel   : ${YELLOW}ringqtunnel-status${NC}"
