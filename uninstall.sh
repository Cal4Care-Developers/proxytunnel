...
# API auth port moved from a dedicated 8443 to the standard 443 -- install.sh
# now adds a single merged rule for 443 (covers both the tunnel API and
# general HTTPS), so removal matches that. Also try the old 8443 rule/
# comment in case this is uninstalling a version from before the port move.
...
# =============================================================================
# STEP 5b -- Remove ringqtunnel-status command
# =============================================================================
section "Removing Status Command"

STATUS_CMD="/usr/local/bin/ringqtunnel-status"
if [[ -f "${STATUS_CMD}" ]]; then
    rm -f "${STATUS_CMD}"
    ok "Removed: ${STATUS_CMD}"
else
    skip "${STATUS_CMD}"
fi
...
echo -e "    - iptables rules added by RingQ installer
    - Status command /usr/local/bin/ringqtunnel-status"
