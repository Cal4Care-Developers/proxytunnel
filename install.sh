...
read_yaml() {
    # read_yaml KEY FILE
    # Anchored sed so values containing colons (e.g. https:// URLs) are kept
    # intact. The old greedy s/.*:// stripped everything up to the LAST colon,
    # leaving only the port number for pbx-api-url (e.g. "8443" not the full URL).
    local key="$1" file="$2"
    grep -m1 "^[[:space:]]*${key}:" "${file}" 2>/dev/null |         sed "s/^[[:space:]]*${key}:[[:space:]]*//" |         tr -d $'\r"\'' || echo ""
}
