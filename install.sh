...
# Exclude ONLY: loopback (127.x), link-local (169.254.x), Docker bridge (172.17-19.x)
# Real WAN interfaces (172.16.x.x, 10.x.x.x) are INCLUDED so user can pick the
# correct one. The old filter excluded ALL 172.16-31.x.x which wrongly hid real NICs.
mapfile -t ALL_LAN_IPS < <(ip -4 addr show scope global 2>/dev/null | \
    grep -oP '(?<=inet\s)\d+(\.\d+){3}' | \
    grep -Ev '^(127\.|169\.254\.|172\.(1[7-9]|2[0-9]|3[01])\.)' || true)
...
NEED_PKG=()
for pkg in git curl ca-certificates iptables-persistent netfilter-persistent file iproute2 nftables; do
...
if $NEED_BUILD; then
    ...
    for candidate in "${BUILD_DIR}/${BINARY_NAME}" "${BUILD_DIR}/bin/${BINARY_NAME}"; do
        if [[ -f "${candidate}" ]]; then
            # GitHub strips execute bits on push -- restore before checking.
            chmod +x "${candidate}" 2>/dev/null || true
            # Detect binary via ELF magic bytes (works without 'file' command)
