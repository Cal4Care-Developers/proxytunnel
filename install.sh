...
read_yaml() {
    ...
    # https://domain:8443) that broke the old sed 's/.*://' approach.
...
if ($OPT_YES || $OPT_REINSTALL) && $CONFIG_EXISTS && ! $OPT_RECONFIGURE; then
    ...
    # offered only when it looks like a real URL (not a broken "8443" value).
    AUTO_API_URL="https://${PBX_DOMAIN}:8443"
